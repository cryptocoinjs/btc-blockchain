  ...
  async.waterfall([
    ...
    function (rs, cb) {
      ...
      self.db.getTips(function(tips) {
        tips = tips.filter(function(tipHash) {
          return tipHash.toString('hex') != parent.toString('hex');
        });
    ...
    function(children, cb) { // Update all children to have new heights
      async.each(children, function(childHash, cbEach) {
        self.db.updateHeight(childHash, blockHeight+1, cbEach);
        // TODO: Remove child from list of Roots, if it was on it
      }, cb);
    },
 ...
 * @param {Array}    blocks   Array of objects, each object must have a 'hash', 'parent', and 'data' property
